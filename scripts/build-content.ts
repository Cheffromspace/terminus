...
import type { Post } from '../src/types/blog.js';
...
async function buildContent() {
  try {
    ...
    type PostWithMetadata = Post & { 
      preRenderedHtml: string;
      metadata: { wordCount: number; readingTime: number; }
    };

    // Sort posts
    const sortedPosts = posts.sort((a: PostWithMetadata, b: PostWithMetadata) => {
    ...
    // Generate manifest
    const manifest = {
      posts: sortedPosts.map((post: PostWithMetadata) => ({
        id: post.id,
        title: post.title,
        slug: post.slug,
        date: post.date,
